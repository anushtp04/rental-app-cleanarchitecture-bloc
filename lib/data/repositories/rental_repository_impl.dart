import '../../domain/entities/rental.dart';
import '../../domain/repositories/rental_repository.dart';
import '../data_source/rental_supabase_data_source.dart';
import '../models/rental_model.dart';

class RentalRepositoryImpl implements RentalRepository {
  final RentalSupabaseDataSource supabaseDataSource;

  RentalRepositoryImpl(this.supabaseDataSource);

  @override
  Future<List<Rental>> getAllRentals() async {
    final rentals = await supabaseDataSource.getAllRentals();
    return rentals.map((rental) => rental.toEntity()).toList();
  }

  @override
  Future<Rental> getRentalById(String id) async {
    final rental = await supabaseDataSource.getRentalById(id);
    return rental.toEntity();
  }

  @override
  Future<Rental> createRental(Rental rental) async {
    final rentalModel = RentalModel.fromEntity(rental);
    await supabaseDataSource.cacheRental(rentalModel);
    return rentalModel.toEntity();
  }

  @override
  Future<Rental> updateRental(Rental rental) async {
    final rentalModel = RentalModel.fromEntity(rental);
    await supabaseDataSource.updateCachedRental(rentalModel);
    return rentalModel.toEntity();
  }

  @override
  Future<void> deleteRental(String id) async {
    await supabaseDataSource.deleteRental(id);
  }

  @override
  Future<void> deleteAllRentals() async {
    await supabaseDataSource.deleteAllRentals();
  }

  @override
  Future<List<Rental>> filterRentals({
    DateTime? fromDate,
    DateTime? toDate,
    String? vehicleNumber,
    String? ownerName,
  }) async {
    final rentals = await supabaseDataSource.filterRentals(
      fromDate: fromDate,
      toDate: toDate,
      vehicleNumber: vehicleNumber,
      ownerName: ownerName, // This is actually rentToPerson now
    );
    return rentals.map((rental) => rental.toEntity()).toList();
  }
}

